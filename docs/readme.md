...
| π       | π     | π       | If you put a `π` symbol before a number, the number is read as hexadecimal. For example, `π3F` would be read as `63`.
...
| inString(    | This is similar to the TI-BASIC command. This will return the location of a sub-string. The inputs are where to start searching and the string to search for: `inString(SearchStart,SearchString`. The size of the input string is returned in `Ɵ'` and if there was no match found, 0 is returned.
